...
class MultimodalModel(nn.Module):
    def __init__(self,
        ...
        hidden_size = image_hidden_size + text_hidden_size

        # Projection layers
        self.image_projection_layer = nn.Sequential(nn.Linear(image_hidden_size, image_hidden_size),
                                                    nn.ReLU(),
                                                    nn.Dropout(dropout_prob),
                                                    )
        self.text_projection_layer = nn.Sequential(nn.Linear(text_hidden_size, text_hidden_size),
                                                    nn.ReLU(),
                                                    nn.Dropout(dropout_prob),
                                                    )

        # Fusion layer
        self.fusion_layer = nn.Linear(image_hidden_size + text_hidden_size, hidden_size)
        ...
        # Freeze image and text model, not recommended, results are very bad
    ...
    def forward(self, 
        ...
        #image_representation = self.image_projection_layer(image_representation)
        #text_representation = self.text_projection_layer(text_representation)

        fused_representation = self.fusion_layer(torch.cat((image_representation, text_representation), dim=1)) 
...
if __name__ == '__main__':
    for seed in [0]:
        run = wandb.init(project="Test_Clef", name="early_fusion_model")
